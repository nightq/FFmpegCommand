...
int transcode_video_for_timehut(int argc, char **argv)
{
    int ret;
    int64_t ti;
...
//     }
    ...
    int transcode_result = 0;
    if ((transcode_result = transcode()) < 0){
        exit_program(1);
    }
    ti = getutime() - ti;
    if (do_benchmark) {
	...
	int transcode_total_result = received_nb_signals ? 255 : main_return_code;
	exit_program(transcode_total_result);
...
//JNIEnv* localEnv;
char **localCmdArray;
jobject localMainActivity = NULL;
int length;
pthread_t transcodeThread;

//线程中调用java
JavaVM *globlJvm = NULL;
jobject globalObject = NULL;
jobjectArray globlCmdArray;

//这个是线程附着到jvm是获取的，所以不是全局的。
JNIEnv *threadEnv;
jclass threadMainActivityClazz;
jmethodID threadLogTranscode;
jmethodID threadLogJava;

long currentTranscodeThreadId = 0;
 ...
 * vl 值数组
 */
static void av_log_callback(void* ptr, int level, char const* fmt, va_list vl)
{
//	__android_log_print(ANDROID_LOG_ERROR, "transcode", "%s", fmt);
 ...
 * just for thread
 */
void logJNI(char* content) {
	if (threadLogTranscode == NULL || threadEnv == NULL
			|| threadMainActivityClazz == NULL || globalObject == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "SOME CLASS IS EMPTY", "fail");
		return;
	}
	jstring encoding = (*threadEnv)->NewStringUTF(threadEnv, content);
//	(*threadEnv)->CallStaticVoidMethod(threadEnv, threadMainActivityClazz,
//			threadLogTranscode, encoding);
	(*threadEnv)->CallVoidMethod(threadEnv, globalObject, threadLogJava,
			encoding);
	/* delete local reference */
	(*threadEnv)->DeleteLocalRef (threadEnv, encoding);

}

void exit_TimeHut_transcode (int result) {
	if ( result != 0) {
		//fail
		__android_log_print(ANDROID_LOG_ERROR, "exit_TimeHut_transcode", "fail");
		logJNI("ANDROID_FOR_TIMEHUT_TRANSCODE_FAIL");
	} else {
		//success
		__android_log_print(ANDROID_LOG_ERROR, "exit_TimeHut_transcode", "success");
		logJNI("ANDROID_FOR_TIMEHUT_TRANSCODE_SUCCESS");
	}
	if(globlCmdArray != NULL && threadEnv != NULL) {
		(*threadEnv)->DeleteGlobalRef(threadEnv, globlCmdArray);
		localCmdArray = NULL;
	}
	//Detach主线程
	if ((*globlJvm)->DetachCurrentThread(globlJvm) != JNI_OK) {
		//	         LOGE("%s: DetachCurrentThread() failed", __FUNCTION__);
	} else {
	}
	//只有result也就是失败的时候才需要停止线程，不然都不停止
//	if (result != 0) {
		pthread_exit(0);
//	}
}

void *transcodeVideoForTimehutForJni(void *args) {
	//Attach主线程
	currentTranscodeThreadId = pthread_self();
	__android_log_print(ANDROID_LOG_ERROR, "transcodeVideoForTimehutForJni",
			"current id : %ld", currentTranscodeThreadId);
	if ((*globlJvm)->AttachCurrentThread(globlJvm, &threadEnv, NULL) != JNI_OK) {
		return NULL;
	}

//	__android_log_print(ANDROID_LOG_ERROR, "AttachCurrentThread", "success");
	//找到对应的类

//	__android_log_print(ANDROID_LOG_ERROR, "get threadMainActivity tmpClass", "success");
	if (threadMainActivityClazz == NULL) {
		jclass tmpClass = (*threadEnv)->GetObjectClass(threadEnv,
				globalObject);
		threadMainActivityClazz = (*threadEnv)->NewGlobalRef(threadEnv,
				tmpClass);
//		__android_log_print(ANDROID_LOG_ERROR, "threadMainActivityClazz",
//				"null");
	} else {
//		__android_log_print(ANDROID_LOG_ERROR, "threadMainActivityClazz",
//				"not null");
	...
	//再获得类中的方法
	if (threadLogJava == NULL) {
		threadLogJava = (*threadEnv)->GetMethodID(threadEnv,
				threadMainActivityClazz, "log", "(Ljava/lang/String;)V");
	}
	if (threadLogTranscode == NULL) {
		threadLogTranscode = (*threadEnv)->GetStaticMethodID(threadEnv,
				threadMainActivityClazz, "logTranscode",
				"(Ljava/lang/String;)V");
	}

	dealCmd(threadEnv, globalObject, globlCmdArray);

	int ret = transcode_video_for_timehut(length, localCmdArray);
	if (ret == 0) {
//		__android_log_print(ANDROID_LOG_ERROR, "transcode_video_for_timehut", "success");
		//success
		exit_TimeHut_transcode(0);
		return NULL;
	}
	error:
	//Detach主线程
	exit_TimeHut_transcode(1);
	return NULL;
}
...
void dealCmd (JNIEnv* env,
	...
	int i = 0;
	for (i = 0; i < length; i++) {
		jstrElement = (jstring) (*env)->GetObjectArrayElement(env, stringArray,
				i);
		localCmdArray[i] = (*env)->GetStringUTFChars(env, jstrElement, 0);
//		__android_log_print(ANDROID_LOG_ERROR, "dealCmd", "cmd line :%s  %d",
//				localCmdArray[i], localCmdArray[i]);
	}
}
...
//	localEnv = env;
	setJNIEnv(env, this);
	globlCmdArray = (*env)->NewGlobalRef(env, stringArray);
	av_log_set_callback(av_log_callback);

	/*创建线程*/
	if ((pthread_create(&transcodeThread, NULL, transcodeVideoForTimehutForJni,
			env)) != 0) {
		__android_log_print(ANDROID_LOG_ERROR, "thread", "fail");
	} else {
		__android_log_print(ANDROID_LOG_ERROR, "thread", "success");
	}
}
...
void setJNIEnv(JNIEnv* env,
		jobject obj) {
	//保存全局JVM以便在子线程中使用
	(*env)->GetJavaVM(env, &globlJvm);
	if (globalObject == NULL) {
		globalObject = (*env)->NewGlobalRef(env, obj);
	}
}
